...
public class TestGetAllPhotosByPhotographer extends BaseSetup {
  ...
  @Test
  public void showAllPhotosToUserByPhotographer()
    throws JsonParseException, JsonMappingException, IOException {
    String getAllPhotosInResponse = given()
      .when()
      .get("/get-photographers-with-photos")
      .then()
      .statusCode(200)
      .extract()
      .response()
    ...
    List<Photographer> photographers = mapper.readValue(
      getAllPhotosInResponse,
      new TypeReference<List<Photographer>>() {}
    );

    for (int index = 0; index < photographers.size(); index++) {
      if (photographers.get(index).getphotographerId().equals(1234)) {
        ...
        Assert.assertEquals(
          photographers.get(index).getphotographerName(),
          "test"
        );
        Assert.assertEquals(
          photographers.get(index).getphotographerUrl(),
          "@test"
        );
